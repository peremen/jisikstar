...
DictDBIndex1Reader::DictDBIndex1Reader(QObject *parent) : QObject(parent) {
}

DictDBIndex1Reader::DictDBIndex1Reader(const QString &filename, QObject *parent): QObject(parent) {
    this->load(filename);
}

bool DictDBIndex1Reader::load(const QString &filename) {
    QFile f;
    ...
    f.close();
    this->populatePreindex();
    this->populate10KIndex();
    this->populateDictionaryData();
    return true;
}

void DictDBIndex1Reader::populatePreindex() {
    ...
    this->wordTo10KIndex.reserve(this->header_data.index_10kb_count * 1000);
    for (int i = 0; i < (int)this->header_data.preindex_1_count; ++i) {
        f.seek(this->header_data.compindex_1_start + this->preindex.at(i).index_pos);
        ...
        while (1) {
            ...
            this->wordTo10KIndex.append(qMakePair(d.desc, d.data_file_pos));
            if (pos >= decompressed_index.size()) break;
        }
        this->preindex[i].index_loaded = true;
    }
    f.close();
}

void DictDBIndex1Reader::populate10KIndex() {
    ...
    this->tenKHashMap.reserve(this->header_data.index_10kb_count * 1000);
    for (int i = 0; i < this->header_data.index_10kb_count; ++i) {
        f.seek(this->header_data.compindex_1_end + i * 2 * sizeof(quint32));
        ...
        for (int j = 0; j < tenk_idx_count; ++j) {
            if (this->is_len_10k_uint32) {
                ...
                len_32 = *(quint32 *)(decompressed_t.constData() + 12 * j + 4);
...
void DictDBIndex1Reader::populateDictionaryData() {
    quint16 decompressed_size_16, compressed_size_16, dummy_16;
...
bool DictDBIndex1Reader::exportToFile(const QString &destination, Snotra::FileType type) {
