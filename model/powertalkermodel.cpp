#include "include/powertalkermodel.h"
...
PowerTalkerModel::PowerTalkerModel(QObject *parent) : QAbstractItemModel(parent) {
    root_data = new Snotra::dict_data;
}

PowerTalkerModel::PowerTalkerModel(const QString &filename, Snotra::database_args args, QObject *parent): QAbstractItemModel(parent) {
    root_data = new Snotra::dict_data;
    this->is_loaded = false;
    this->load(filename, args);
}

PowerTalkerModel::~PowerTalkerModel() {
    delete root_data;
}


bool PowerTalkerModel::load(const QString &filename, Snotra::database_args args) {
...
bool PowerTalkerModel::get_load_status() {
    return this->is_loaded;
}

void PowerTalkerModel::clear_root_data() {
...
QModelIndex PowerTalkerModel::index(int row, int column, const QModelIndex &parent) const {
...
QModelIndex PowerTalkerModel::parent(const QModelIndex &child) const {
...
int PowerTalkerModel::rowCount(const QModelIndex &parent) const {
    Snotra::dict_data *parent_data;
...
int PowerTalkerModel::columnCount(const QModelIndex &) const {
    return 1;
}

bool PowerTalkerModel::hasChildren(const QModelIndex &parent) const {
...
QVariant PowerTalkerModel::data(const QModelIndex &index, int role) const {
...
QByteArray PowerTalkerModel::get_data(const QModelIndex &index) const {
    ...
    t = f.read(len);
    LZMADecoder::decode_lzma_2(t, decompressed_t);
    f.close();
    pos = 0;
...
QVariant PowerTalkerModel::headerData(int , Qt::Orientation orientation, int role) const {
    switch (role) {
    case Qt::DisplayRole:
...
Qt::ItemFlags PowerTalkerModel::flags(const QModelIndex &index) const {
    if (!index.isValid()) return 0;
    return Qt::ItemIsEnabled | Qt::ItemIsSelectable;
}

bool PowerTalkerModel::canFetchMore(const QModelIndex &) const {
    return false;
}

void PowerTalkerModel::fetchMore(const QModelIndex &) {
    return;
}
